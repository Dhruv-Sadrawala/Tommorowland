...
CREATE TABLE NEW_STORAGE(eno number(10),old_salary number(10),new_salary number(10));
...
CREATE OR REPLACE TRIGGER WROK_IT
AFTER INSERT ON emp_t
FOR EACH ROW
BEGIN
IF :NEW.dept='IT' THEN
INSERT INTO EMPINS VALUES(:NEW.e_no,:NEW.ename,:NEW.dept,:NEW.salary);
ELSE
INSERT INTO emp_t VALUES(:NEW.e_no,:NEW.ename,:NEW.dept,:NEW.salary);
END IF;
END;
/

--Write a trigger which will store eno,old_salary,new_salary into new_emp table before 
--update the salary in emp table.

CREATE OR REPLACE TRIGGER NEW_STORAGE
BEFORE UPDATE ON emp_t
FOR EACH ROW
BEGIN
INSERT INTO NEW_STORAGE VALUES(:OLD.e_no,:OLD.salary,:NEW.salary);
END;
/